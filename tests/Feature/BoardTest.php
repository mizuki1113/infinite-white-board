...
class BoardTest extends TestCase
{
    ...
    public function test_web_routes_manage_boards(): void
    {
        ...
        $this->get("/boards/{$boardId}")
            ...
            ->assertSee('id="stroke-width"', false)
            ->assertSee('id="brush-style"', false)
            ->assertSee('id="eraser-size"', false)
            ->assertSee('destination-out')
            ->assertSee('value="highlighter"', false)
            ->assertSee('id="undo"', false)
            ->assertSee('const historyLimit = 50')
            ->assertSee('event.key.toLowerCase()')
            ->assertSee('This freehand line is open. Draw a closed loop to fill it.')
            ->assertSee('pointInPolygon')
            ->assertSee('distance')
            ->assertSee('isFreehand: true')
